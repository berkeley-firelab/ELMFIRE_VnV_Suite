...
ELMF_BIN=$(awk '/^elmfire:/,/^postprocess:/' "$YAML" | awk -F 'bin:' 'NF>1{gsub(/[ \"]/, "", $2); print $2}')
ELMF_CFG=$(awk '/^elmfire:/,/^postprocess:/' "$YAML" | awk -F 'config:' 'NF>1{gsub(/[ \"]/, "", $2); print $2}')
RUNTIME_LIMIT=$(awk '/^elmfire:/,/^postprocess:/' "$YAML" | awk -F 'runtime_limit_s:' 'NF>1{gsub(/[ \"]/, "", $2); print $2}')


mkdir -p "$CASE_DIR/output" "$CASE_DIR/figures" "$CASE_DIR/logs" "$CASE_DIR/report"
...
SECS_START=$(date +%s)
set +e
"$ELMF_BIN" "$CASE_DIR/$ELMF_CFG" > "$CASE_DIR/logs/elmfire.stdout" 2> "$CASE_DIR/logs/elmfire.stderr"
RC=$?
set -e
SECS_END=$(date +%s)
