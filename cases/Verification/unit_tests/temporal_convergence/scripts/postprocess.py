...
REFERENCE_TIME_GRID = 1440
ANALYTICAL_REFERENCE_RESOLUTION = 4096
DEFAULT_ROS = 0.0116332 #* 0
DEFAULT_R0 = 5.0
TIMESTEP = -1
TFINAL = 360
...
def simulation_configs() -> list[tuple[int, Path, Path]]:
	configs: list[tuple[int, Path, Path]] = []
	for time_grid in TIME_GRIDS:
		config_path = INPUTS_DIR / f"elmfire_{time_grid}.data"
		output_dir = OUTPUTS_ROOT / str(time_grid)
		if config_path.exists() and output_dir.exists():
			configs.append((time_grid, config_path, output_dir))
	return configs
...
def find_timestep_at_time(times: np.ndarray, series: np.ndarray, target_time: float) -> tuple[float, np.ndarray]:
	"""Find the timestep closest to target_time and return (actual_time, data)."""
	if series.size == 0 or times.size == 0:
		raise FileNotFoundError("No time series data available")
	if series.shape[0] != times.shape[0]:
		raise ValueError(
	...
	return float(times[idx]), series[idx]
...
def save_convergence_plot(results: list[dict[str, object]], l2_curve: bool = True, linf_curve: bool = False) -> None:
	grids = np.array([int(r["time_grid"]) for r in results], dtype=float)
	dt_vals = np.array([float(r["dt"]) for r in results], dtype=float)
	linf = np.array([float(r["linf_rel"]) for r in results], dtype=float)
	l2 = np.array([float(r["l2_rel"]) for r in results], dtype=float)

	order2_ref = l2[0] * (dt_vals / dt_vals[0]) ** 2

	plt.figure(figsize=(8, 6))
	plt.xscale("log", base=2)
	plt.yscale("log")
	if l2_curve:
		plt.plot(dt_vals, l2, marker="o", label=r"$L_2$")
	if linf_curve:
		plt.plot(dt_vals, linf, marker="s", label=r"$L_\infty$")
	plt.plot(dt_vals, order2_ref, linestyle="--", color="black", label=r"$O(\Delta t^2)$ reference")
	# for x_val, y_val, grid in zip(grids, linf, grids, strict=False):
	#     plt.annotate(f"{int(grid)}", (x_val, y_val), textcoords="offset points", xytext=(5, 5), fontsize=8)
	plt.xlabel(r"$\Delta t$")
	plt.ylabel("Error")
	plt.title("Temporal convergence against analytical solution")
...
def main() -> None:
	sim_configs = simulation_configs()
	if not sim_configs:
		raise FileNotFoundError(f"No simulation configs found under {INPUTS_DIR}")

	reference_config_path = next(
		(config_path for time_grid, config_path, _ in sim_configs if time_grid == REFERENCE_TIME_GRID),
		sim_configs[0][1],
	)
	ref_config = parse_elmfire_data_config(reference_config_path)
	reference_output_dir = next(output_dir for time_grid, _, output_dir in sim_configs if time_grid == REFERENCE_TIME_GRID)

	ref_times, ref_phi_series = load_time_series(reference_output_dir, "phi")
	if ref_phi_series.size == 0:
		raise FileNotFoundError(f"No phi outputs found in {reference_output_dir}")

	ref_phi0 = read_tif_as_array(INPUTS_DIR / "phi.tif").astype(np.float32, copy=False)
	...
	t_reference, _ = find_timestep_at_time(ref_times, ref_phi_series, TFINAL)

	analytical_nx = ANALYTICAL_REFERENCE_RESOLUTION
	analytical_ny = ANALYTICAL_REFERENCE_RESOLUTION
	...
	for time_grid, config_path, output_dir in sim_configs:
		...
		print(f"[dt={time_grid}], dt: {dt:.6f}, Time: {latest_time:.2f}s, RMSE: {rmse:.6f}, L_inf: {linf:.6f}, L2: {l2:.6f}, L_inf_rel: {linf_rel:.6f}, L2_rel: {l2_rel:.6f}")

		plot_field(
			x,
			y,
			phi_final,
			f"Final $\\phi$, $N_t = {time_grid}$",
			FIG_DIR / f"phi_final_{time_grid}.png",
			vmin=-1.0,
			vmax=1.0,
		)
		plot_field(
			...
			y,
			error,
			f"Error, $N_t = {time_grid}$",
			FIG_DIR / f"phi_error_{time_grid}.png",
			cmap="coolwarm",
		)
	...
	if not results:
		raise FileNotFoundError("No usable simulation outputs were found")
	...
	np.savez_compressed(
		...
		time_grid=np.array([r["time_grid"] for r in results], dtype=float),
		resolution=np.array([r["resolution"] for r in results], dtype=float),
		dt=np.array([r["dt"] for r in results], dtype=float),
	...
	print(f"  - Reference time grid: {REFERENCE_TIME_GRID}")
