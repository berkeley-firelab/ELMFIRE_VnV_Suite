...
FLOAT_RASTERS = [
	...
	("adj", 0.0),  # Spread rate adjustment factor
...
FUEL_MODELS_FILENAME = "fuels_models.csv"
...
def get_required_env_float(name: str) -> float:
	...
	try:
		...
		raise RuntimeError(f"Environment variable {name} must be numeric, got: {value}") from exc
...
def main() -> None:
	...
	# Prefer environment overrides, otherwise read values from elmfire.data.in.
	if os.getenv("DOMAINSIZE") is not None:
		domain_size = get_required_env_float("DOMAINSIZE")
	else:
		xllcorner = get_float_value(config, "COMPUTATIONAL_DOMAIN_XLLCORNER")
		if xllcorner >= 0:
			raise RuntimeError(
				"Cannot infer DOMAINSIZE from elmfire.data.in when "
				"COMPUTATIONAL_DOMAIN_XLLCORNER is non-negative. "
				"Set DOMAINSIZE in the environment."
			)
		domain_size = -2.0 * xllcorner

	if os.getenv("CELLSIZE") is not None:
		cell_size = get_required_env_float("CELLSIZE")
	else:
		cell_size = get_float_value(config, "COMPUTATIONAL_DOMAIN_CELLSIZE")

	a_srs = os.getenv("A_SRS", config.get("A_SRS", "EPSG:32610"))
