...
the accumulated deposition solely as a numerical artifact.
...
\]
ELMFIRE applies \texttt{EMBER\_GR\_PER\_MW\_VEGE} to the total fire power of
...
\]
The configured \texttt{EMBER\_GR\_PER\_MW\_VEGE} values are consequently
6.66, 3.33, and 1.11~pcs/s/MW for the 5, 10, and 30~m variants. In all three
variants,
...
while the flame resides in that cell~\cite{Qin2025}. In the current implementation,
\texttt{READ\_SPOTTING} reads the \texttt{PER-MW} source parameters,
\texttt{SPOTTING} calls \path{EMBER_TO_EMIT_PER_CELL} to convert
fireline intensity and cell width to an ember rate, and
\path{CALC_SPOTTING_DURATION} determines the source lifetime from the
cell fire state. Accumulated deposition, fireline intensity, and spread-rate
rasters are then written for postprocessing. The report checks both halves of
the discretization: preprocessing keeps the represented one-metre source
strength intensive by scaling the configured rate with cross-stream cell size,
and postprocessing derives the expected density from the simulated FLIN and VS
at the same physical state. A source-duration or pixel-power scaling defect
would therefore create a systematic resolution trend. This is a controlled
code-verification test; it does not validate the empirical generation constant
or the transport distribution against experiments.
...
interior of the latest \texttt{flin\_*\_*.tif} and reads the spread rate from
the corresponding cell of \texttt{vs\_*\_*.tif}. ELMFIRE writes these fields
...
\[
  ...
  \quad [\mathrm{pcs\,m^{-2}}].
\]
For the current outputs, the mean values across the variants are
...
The per-variant values and selected raster cells are retained in
...
\end{array}
\]
in \(\mathrm{pcs\,s^{-1}\,MW^{-1}}\) for the last two rows. The 5- and 10-m CFL steps divide the requested 500-s duration exactly. At 30~m, the nominal CFL step is 2.235469~s; preprocessing selects the conservative whole-second divisor 2~s, giving 250 steps and preserving the 500-s stop while keeping \(CFL_w=0.4473<0.5\). Thus the
source-scaling reference is exactly 33.3 in all three variants, and its
relative error is
\[
...
obtained by cancellation in one variant.  Completeness and the conjunctive
...
\midrule
Output completeness & Final \path{ember_flux}, \path{flin}, and \path{vs} rasters for each manifest variant; shape, cell size, nodata, and halo are validated. & 3 of 3 variants usable \\
...
Far-field mean relative error & \(E_r=\operatorname{mean}_{150<x\le200}|n_i''-n''_{\mathrm{ref}}|/n''_{\mathrm{ref}}\), evaluated separately for every resolution. & \(E_r\le0.05\) for every variant \\
...
The preprocessor creates flat, uniform FBFM102 domains at
...
For each completed variant, the mean accumulated density over
$150<x\leq200$~m is compared with that variant's FLIN/VS-derived reference.
The mean relative error must not exceed 5\%. The case passes only when all
three variants have valid outputs and pass independently. Agreement across the
...
Figure~\ref{fig:domain-result} shows the representative accumulated firebrand density from \texttt{dx10}.
...
line uses the FLIN and spread rate extracted from that same variant.
...
Variants configured & \Metric{numvariants} \\
Variants computed & \Metric{numvariantscomputed} \\
\bottomrule
\end{tabular}
\caption{Biomass emission-time verification summary.}
\end{table}

The overall decision is sourced from the generated metric macros and is \textbf{\Metric{status}}.
...
This case isolates a uniform, one-dimensional transport problem with crosswind dispersion, delayed ignition, and secondary spotting disabled. It verifies emission-time scaling and accumulated deposition, but does not by itself validate stochastic trajectories or ignition physics. The preprocessing parameters, variant manifest, calculated raster samples, and metric values are retained with the case so that the reported decision can be reproduced.
