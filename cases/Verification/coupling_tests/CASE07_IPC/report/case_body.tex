...
\end{array}
\]
For each resolution the conservative CFL-derived timestep defines the probability transformation and cross-resolution control. Preprocessing selects \(N=\lceil240/\Delta t_{CFL}\rceil\), writes the conservative divisor \(\Delta t=240/N\), and keeps \(t_{stop}=240\)~s exactly; the manifest records nominal and actual CFL values plus the integer step count. This shows why the step probability changes while the continuous delay CDF
does not.
