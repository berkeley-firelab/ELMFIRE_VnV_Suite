...
The test exposes grid- or CFL-dependent advection speed, lost transient output, and inconsistent front extraction. Success requires the measured leading-edge rate to approach the prescribed wind speed over the accepted grid and CFL range.

\section{Mathematical formulation and numerical implementation}

\begin{sloppypar}
This sweep reconstructs the wind-CFL convergence experiment~\cite{Qin2025}. \texttt{READ\_SPOTTING} selects \texttt{PER-AREA}, \texttt{EULERIAN}, and \texttt{DIRECT}; \texttt{SPOTTING} creates the source, \texttt{EMBER\_\allowbreak TRAJECTORY\_\allowbreak EULERIAN} advances it, and \texttt{EULERIAN\_\allowbreak SPOTTING\_\allowbreak MAIN} converts the arriving ember state into a new ignition while updating \texttt{TIME\_\allowbreak OF\_\allowbreak ARRIVAL} and the level set. Because surface spread is suppressed, sustained propagation must traverse that complete code path. The wind-CFL sweep changes only the dimensionless ratio of wind displacement per step to cell size while the physical domain and areal source remain fixed. Consequently, a transport-stencil, timestep, direct-ignition, or TOA update error appears as resolution- or CFL-dependent leading-edge speed. The comparison to the wind speed is an idealized numerical reference and does not claim that real firebrands travel deterministically at the ambient wind speed.
...
leading-edge location, and stepwise ROS.

Every variant with $CFL_{wind}\leq0.8$ passes when
\begin{equation}
...
all 20 stable-regime variants guards against compensating resolution and
...
\midrule
Required-output completeness & Transient \path{phi} rasters, dump times, and at least one valid front-rate interval between 300 and \SI{1500}{m}. & All 20 variants with \(CFL_{wind}\le0.8\) \\
Normalized mean-ROS error & \(E_U=|\overline{ROS}/u_{wind}-1|\), where \(\overline{ROS}\) is displacement divided by elapsed time over the comparison interval. & \(E_U\le0.10\) for every required variant \\
Overall decision & All required variants must be complete and satisfy the normalized-speed limit; higher-CFL variants are diagnostic only. & PASS iff all required components pass \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:input-configuration} records the prepared spatial inputs for \texttt{dx10\_cfl0p2}.
...
\begin{itemize}
  \item generation: \texttt{PER-AREA};
  \item spotting distance: \texttt{EMPIRICAL};
  \item accumulation: \texttt{EULERIAN};
  \item ignition: \texttt{DIRECT}.
\end{itemize}
...
wind-based CFL values 0.2, 0.4, 0.5, 0.6, 0.8, 1.0, and 1.2, for 28 variants.  Each
time step is prescribed by
\begin{equation}
  \Delta t=CFL_{wind}\frac{\Delta x}{u_{wind}}.
\end{equation}
All variants retain the same physical domain; only the number of active and
buffer cells changes.

\section{Actual simulation results}

Figure~\ref{fig:domain-result} shows the representative transported level-set field from \texttt{dx10\_cfl0p2}.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth,height=0.45\textheight,keepaspectratio]{../figures/eulerian_transport_convergence.pdf}
...
\SI{\Metric{representativefitslope}}{m.s^{-1}}, but it is not the reported
acceptance metric.
...
$\overline{ROS}=\SI{\Metric{representativemeanros}}{m.s^{-1}}$, or
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth,height=0.72\textheight,keepaspectratio]{../figures/representative\_front_extraction.pdf}
...
Prepared variants & \Metric{numvariants} \\
Variants with simulation results & \Metric{numvariantscomputed} \\
Required variants & \Metric{numrequiredvariants} \\
Required variants evaluated & \Metric{numrequiredvariantscomputed} \\
...
$ROS/u_{wind}=\Metric{coarsecflofiveNormalizedros}$, corresponding to a
...
From this case directory, configure \texttt{ELMFIRE\_BIN} and run \texttt{./run\_case.sh}. The runner preprocesses the manifest, executes every variant sequentially with isolated outputs and logs, postprocesses the results, and compiles the standalone report.
