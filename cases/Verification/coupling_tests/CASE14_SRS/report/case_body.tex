...
firebrands.  The global \texttt{NO\_SURFACE\_FIRE} switch remains true to suppress vegetative surface-fire propagation.  The current source independently refreshes initialized model-14 building transient HRR under this setting. The configured time-step scaling is
...
\]
The fractional step is required to hold this spatial sweep at fixed wind CFL.
Preprocessing sets each generated stop to
\(\lceil150000/\Delta t\rceil\Delta t\), then records the requested stop,
aligned stop, and integer step count in the manifest.
...
The primary publication provides the independent propagation oracle used only
...
oracle does not replace the current-run metrics or authorize accepting an
...
All 15 current-fingerprint ELMFIRE processes reached the source success
...
20, 30, and 40~m, and the publication-derived no-propagation oracle permits the
...
and rejection reasons are recorded per variant in
\path{outputs/metrics.json}. The overall decision calculated by postprocessing is
\textbf{\OverallStatus}.
...
\CompletedVariantCount{} of \RequestedVariantCount{} variants.  The 10-m
...
publication reports no propagation at 23 and 27~m, but that oracle does not
