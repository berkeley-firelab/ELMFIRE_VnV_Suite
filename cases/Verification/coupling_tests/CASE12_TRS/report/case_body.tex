...
is regenerated from case-local simulation products.

\textbf{Case ID:} \CaseID

This case verifies temporal-resolution sensitivity in a firebrand-driven WUI calculation. Spatial resolution, geometry, wind, heat-release history, firebrand physics, random seed, and output cadence are fixed; only the level-set timestep changes over a wide CFL range.
...
\]
The inner integral is evaluated using the lognormal CDF and the time integral
...
In the generated namelists, \texttt{SIMULATION\_DT} and
\texttt{SIMULATION\_DTMAX} impose the four values above. The input
\texttt{TARGET\_CFL} remains the ordinary surface-spread limiter and does not
redefine the four wind-based temporal variants.  The global \texttt{NO\_SURFACE\_FIRE} switch remains true to suppress vegetative surface-fire propagation.  The current source independently refreshes initialized model-14 building HRR, and the nonburnable fuel-93 background provides an additional isolation of building-only spread.
...
requested stop, aligned stop, and integer step count in the manifest.
...
\end{array}
\]
which reproduces the variant names to their displayed precision.
...
is unchanged among variants, so its ideal load error is zero.  The explicit
...
exceed 5\% for CFL=1 and 10. Every variant must be complete, and every
...
support a defensible tighter decision.  All four variants are required so the
...
\midrule
Output completeness & Current-fingerprint outputs for baseline and CFL 1, 10, and 200 variants. & 4 of 4 variants \\
...
Figure~\ref{fig:input-configuration} records the prepared spatial inputs for \texttt{cfl0p5\_dt0p28}.
...
\begin{table}[H]
\centering\small
\caption{Generated temporal-resolution variants. The time step is calculated
from \(\Delta t=\mathrm{CFL}\,\Delta x/U\), not rounded to the display value.}
\begin{tabular}{lrrr}
\toprule
Variant & Wind-based CFL & \(\Delta t\) (s) & \(t_{\max}\) (s)\\
\midrule
\VariantRows
\bottomrule
\end{tabular}
...
All variants use \(\Delta x=\Delta y=\SI{10}{m}\), alternating
...
\texttt{scripts/preprocess.py} defines its customizable constants at the top,
creates one self-contained directory per variant, writes co-registered UTM
GeoTIFFs and case-local fuel-model tables, marks every structure footprint in
the first upwind column with negative \(\phi\), and records the exact time step
and an SHA-256 input fingerprint in \texttt{variants/manifest.json}. Unique
structure IDs preserve the two-dimensional lattice while the postprocessor
extracts a representative cross-stream row for common structure profiles.

\texttt{run\_case.sh} prepares all variants, runs the executable given by the
absolute \texttt{ELMFIRE\_BIN} path, captures per-variant logs, records the
successful input fingerprint, invokes postprocessing, and compiles this
standalone report. Resume mode skips a run only when its output fingerprint
matches the current generated inputs.

\texttt{scripts/postprocess.py} aggregates final ember counts and ignition times
by physical structure ID, reconstructs cumulative histories from transient
ember rasters and \texttt{dump\_times} metadata, evaluates the analytical deposited-load relation,
...
Figure~\ref{fig:domain-result} shows the representative time-of-arrival field from \texttt{cfl0p5\_dt0p28}.
...
current-fingerprint runs and all tabled acceptance checks. A not-run state is
...
current executable does not write the internal \texttt{EMBER\_TOA} array as a
final raster; that diagnostic is therefore intentionally excluded from the
decision contract.

\section{Technical interpretation}
With the declared building-only switch restored, all four current-source variants produce nonzero structural heat release, firebrand transport, fingerprint-matched final rasters, and a complete metric record.  This invalidates the earlier zero-HRR and missing-output interpretations, both of which arose while diagnosing the stale executable or the temporary altered switch.
