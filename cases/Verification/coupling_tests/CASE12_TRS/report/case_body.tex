...
\SI{40}{mph} from west to east, and surface-fire spread is disabled. Thus a
...
\texttt{TARGET\_CFL} remains the ordinary surface-spread limiter; with
\texttt{NO\_SURFACE\_FIRE=.TRUE.}, it does not redefine the four wind-based
temporal variants.
...
distribution, and no surface fire. The predicted result is near-overlap for
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{temporal_resolution_profiles.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\linewidth]{temporal_resolution_error.pdf}
...
History-shape agreement with absolute-load disagreement distinguishes temporal integration behavior from source normalization or analytical-reference errors. The component metrics retain that distinction instead of reducing the result to visual similarity.
