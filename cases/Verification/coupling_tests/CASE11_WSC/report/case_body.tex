...
This case verifies the effect of nonzero surface spread on ignition synchronization in a fine-grid WUI calculation. A no-surface reference and a surface-coupled variant use identical geometry, wind, firebrand physics, and resolution; only surface propagation changes.
...
structure heat-release rate used by the firebrand model is
\[
...
In the no-surface variant, cell-local deposition makes downwind cells within
one structure
...
The surface-coupled variant retains the firebrand process but activates surface
...
Surface propagation crosses a represented structure much faster than independent firebrand accumulation ignites its cells. The coupled variant should therefore ignite earlier and with a smaller within-structure time span, while the unchanged deposition algorithm should preserve measurable load nonuniformity.
...
a smaller coupled within-structure span, and a larger coupled fitted ROS,
...
variants, let \(t_{\mathrm{ns},j}\) denote the no-surface arrival and
...
lower-bound test: the surface-coupled variant is expected to improve ignition
synchronization,
...
not an accuracy tolerance.  Requiring both current-fingerprint variants is
...
\midrule
Output completeness & Both variants must carry the current input fingerprint and usable structure-level load/ignition fields. & 2 of 2 variants \\
...
Figure~\ref{fig:input-configuration} records the prepared spatial inputs for \texttt{surface\_coupled\_dx2p5}.
...
\begin{tabular}{lll}
\toprule
Parameter & No-surface variant & Surface-coupled variant\\
\midrule
...
The fixed wind CFL requires a fractional step. Preprocessing sets each generated
stop to \(\lceil t_{\mathrm{requested}}/\Delta t\rceil\Delta t\), and records
the requested stop, aligned stop, and integer step count in the manifest.
...
The surface-coupled variant stops
after all structures have ignited and a mature, nonuniform ember-load field
has formed. Since TOA is immutable after ignition and load distortion is
measured within the coupled run rather than between terminal times, this
difference reduces runtime without changing any acceptance quantity.

The expected signature is not agreement with a prescribed straight-line TOA
curve. It is a directional paired result: the surface-coupled variant must
ignite earlier, have
...
\texttt{scripts/preprocess.py} defines customizable parameters at its
beginning, creates two self-contained variant directories, and writes
co-registered UTM GeoTIFFs. Square structures and surface-fuel gaps repeat in
both horizontal directions across the \(100\times100\)-m physical domain; only
the two-cell numerical halo uses inert model 93. Structure cells also receive
co-registered building-property rasters.
ELMFIRE therefore applies the Hamada WUI velocity in structures and the
Rothermel velocity in surface gaps, giving the coupled level-set front a
continuous two-dimensional path. Every structure in the first upwind column is
initialized, and a structure-ID raster preserves each physical \(4\times4\)-cell
footprint independently.
The generated configurations differ only in
\texttt{NO\_SURFACE\_FIRE}. Input fingerprints prevent stale results from
being compared.

\texttt{run\_case.sh} prepares both variants, runs the absolute executable
provided in \texttt{ELMFIRE\_BIN}, captures separate logs, invokes
postprocessing, and compiles this standalone report. The postprocessor reads
the final authoritative TOA and accumulated-ember rasters, normalizes each cell
by its physical \(\Delta x\,w_y\) area, computes all metrics above, and
generates the paired timing-and-load diagnostic figure.

\section{Actual simulation results}

Figure~\ref{fig:domain-result} shows the representative time-of-arrival field from \texttt{surface\_coupled\_dx2p5}.
...
\CompletedVariantCount{} of \RequestedVariantCount{} current-fingerprint
variants have evaluable outputs. Figure~\ref{fig:wsc-reference} first presents
...
current-fingerprint outputs and every synchronization, acceleration, and
