...
this threshold near \SI{2496.9}{s}. The case-local postprocessor preserves the
...
\]
These values are inserted in the cell-CDF convolution at each 5-s midpoint;
...
The global \texttt{NO\_SURFACE\_FIRE} switch remains true to suppress vegetative surface-fire propagation.  In the current source, initialized model-14 buildings independently refresh their transient HRR and therefore remain active under this setting.  The generated non-structure fuel model 93 is also nonburnable, providing a second explicit isolation of the building-only experiment.
Because the wind-CFL bound requires a subsecond step, preprocessing computes the
...
requested duration and an exact timestep multiple; the manifest records nominal
...
ID, evaluates the formulas above, and writes \texttt{outputs/metrics.json}. Before reading either terminal raster it requires the current input fingerprint and successful source termination, parses unique \texttt{SIMULATION\_TSTOP}, \texttt{SIMULATION\_DT}, and \texttt{DT\_METEOROLOGY} controls from the generated namelist, confirms manifest agreement and exact stop divisibility, and requires one ledger final row plus one ember-flux/TOA pair. A final dump beyond the nominal stop is accepted only when subtracting the meteorology interval gives a timestep-grid-aligned solver time within one timestep of the stop; ambiguous, early, off-grid, or duplicate evidence remains not evaluable.
...
The corrected exact-grid baseline and native coarse variant both terminate at
an exact \SI{22000}{s} final dump with matching fingerprints.  The baseline
...
exhausting the shared environment, and no completion fingerprint was written,
...
The integrated variants test whether one physical structure remains the source
