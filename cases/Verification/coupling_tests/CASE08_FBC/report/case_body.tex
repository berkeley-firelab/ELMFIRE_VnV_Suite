...
rate by pixel area before subtracting from \texttt{EMBER\_FLUX}. Only the
consumption-on member calls this routine. Final \texttt{ember\_flux} therefore
represents the active deposited population. The transient
\texttt{ember\_flux\_transient} product records new deposition during an
...
\texttt{MIN\_LIFETIME=1E9}~s, whereas the documented model requires
...
should not materially change TOA. The active-particle line density is recovered from a
...
All 48 manifest variants are required: eight paired 260-s metric runs and 40
consumption-on stop-time runs (four resolutions at ten sample times). Exactly
one final active-flux and TOA raster per variant is required. The two-cell numerical halo is excluded. The components
are:
\begin{enumerate}
...
\midrule
Output completeness & Eight paired metric runs plus 40 active-stock stop-time runs, with final-raster shape/nodata validation and halo exclusion. & 48 of 48 variants \\
...
Figure~\ref{fig:input-configuration} records the prepared spatial inputs for \texttt{dx10\_consumption\_on}.
...
Temporal CFL & 0.5 based on 0.71 m/s & delayed-ignition baseline time refinement \\
...
\bottomrule
\end{tabular}
\caption{Configuration used by every paired variant.}
\end{table}
...
Figure~\ref{fig:domain-result} shows the representative accumulated firebrand density from \texttt{dx10\_consumption\_on}.
...
All 48 variants were rerun. The temporal curves use ten independent
...
variant: active firebrands, arrival time, leading-edge position, and
leading-edge ROS.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth,height=0.66\textheight,keepaspectratio]{../figures/firebrand_consumption_state.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth,height=0.43\textheight,keepaspectratio]{../figures/firebrand_consumption_convergence.pdf}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth,height=0.43\textheight,keepaspectratio]{../figures/firebrand_consumption_verification.pdf}
...
\midrule
Output completeness & 48 variants & \Metric{evaluatedvariants}/\Metric{requiredvariants} & -- \\
...
difference is 0~s, while the normalized profile errors are 0.0326, 0.0846, and
0.3417 at 5, 10, and 30~m, respectively. The source-normalized profiles therefore
...
The 10~s dump cadence now provides the active-stock history near 200~m and
