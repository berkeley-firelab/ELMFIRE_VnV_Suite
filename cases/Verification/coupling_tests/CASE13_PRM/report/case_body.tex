...
\]
Transport is evaluated once for the structure-scale HRR, deposition is
...
parameter point, mean rate of spread is obtained by fitting structure position
...
Required capability & Resolution-independent single-structure source, collector, and input adapter. & Available \\
...
\texttt{scripts/preprocess.py} places customizable filenames and design
constants at its beginning, enumerates all 270 points into
\texttt{outputs/parameter\_design.csv}, writes each variant's co-registered
structure-fraction, structure-ID, fuel-model, and initial-level-set rasters,
and records their fingerprints in the capability manifest. It scans the current Fortran source for
\texttt{USE\_RESOLUTION\_INDEPENDENT\_STRUCTURES}. The case refuses to mark a
point runnable unless that source capability and a matching case adapter both
exist. This guard is necessary because running the native cell-local model
would reproduce the cell-local discretization defect rather than the intended
resolution-independent experiment.

\texttt{scripts/postprocess.py} accepts exact results in
\texttt{outputs/parametric\_results.csv} with the schema declared in
\texttt{case.json}. It checks completeness, derives critical boundaries,
calculates all rank and response metrics, and creates the two classification
maps and two ROS panels. Missing results remain \emph{NOT EVALUATED}.
\texttt{run\_case.sh} prepares the complete design, postprocesses the
capability state, and compiles this standalone report without launching an
inadmissible substitute.

\section{Actual simulation results}

No whole-domain ELMFIRE result is available because the required integrated-structure
capability is absent and no variant is runnable under the current source interface.
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{parametric_response_maps.pdf}
...
The complete parameter design, expected behavior, result schema, metrics,
figure generator, and standalone report are now present. Once the source
feature and its input adapter are implemented, the capability gate should be
