...
def capture_environment() -> dict[str, str]:
    ...
    return {
        ...
        "ELMFIRE version declaration": os.environ.get(
            "ELMFIRE_VERSION", "not declared"
        ),
...
def write_case_includes(path: Path, summaries: list[CaseSummary]) -> None:
    ...
    for summary in summaries:
        heading = f"{latex_escape(summary.case_id)} -- {latex_escape(summary.title)}"
        ...
        lines.extend(
            [
                ...
                rf"  \section*{{{heading}}}",
                r"  \textbf{NOT EVALUABLE:} the standalone case report PDF is missing.",
                r"}",
                "",
            ]
        )
...
def write_summary_table(
    ...
    lines.extend(
        [
            ...
            "summarize the declared executable command, namelist, MPI allocation, "
    ...
    for summary in summaries:
        ...
        lines.append(
            " & ".join(
                (
                    case_link,
                    latex_escape_breakable(summary.elmfire_command),
                    latex_escape_breakable(summary.elmfire_config),
                    latex_escape(summary.mpi_ranks),
                    latex_escape(summary.namelist_schema),
                )
            )
            + r" \\"
        )
    lines.extend(
        [
            ...
            r"\texttt{outputs/metrics.json}. A successful run script or report build is "
            "not treated as scientific evidence of PASS. Missing, incomplete, blocked, "
            "or non-decisional results are reported as NOT EVALUABLE.",
            "",
    ...
    for summary in summaries:
        label = case_label(summary)
        case_link = rf"\hyperref[{label}]{{{latex_escape(summary.case_id)}}}"
        title_link = rf"\hyperref[{label}]{{{latex_escape(summary.title)}}}"
        lines.append(
            " & ".join(
                (
                    case_link,
                    latex_escape(summary.category.replace("_", " ")),
                    title_link,
